...
#define MEMPOOL_BASE 0x1e800000
#define MEMPOOL_SIZE (24*1024*1024)
...
void *Xe_pAlloc(struct XenosDevice *xe, u32 *phys, int size, int align)
{
    ...
    if (phys)
        *phys = (u32) r - (u32)xe->mempool.base + MEMPOOL_BASE; // - virt + phy
...
{
//	XE_PRINT("may kick: wptr = %x, last_wptr = %x\n", rb_secondary_wptr, last_wptr);
...
//		XE_PRINT("uploading shader constants..\n");
...
/*			int i;
			XE_PRINT("float cst count: %d\n",count);
			for(i=0;i<count;++i) XE_PRINT("%f ",c[i]);
			XE_PRINT("\n");*/
		}
...
/*			int i;
			XE_PRINT("int cst count: %d\n",count);
			for(i=0;i<count*2;++i) XE_PRINT("%04x ",((u16*)constants)[i]);
			XE_PRINT("\n");*/
...
//		XE_PRINT("raw: %08x\n", vfetch_patch);
//		XE_PRINT("type=%d, stream=%d, insn=%d\n", type, stream, insn);
		u32 *vfetch = shader_code + insn * 12;
//		XE_PRINT("  old vfetch: %08x %08x %08x\n", vfetch[0], vfetch[1], vfetch[2]);
//		XE_PRINT("    old swizzle: %08x\n", vfetch[1] & 0xFFF);
...
//		XE_PRINT("  old Offset=%08x, DataFormat=%d, Stride=%d, Signed=%d, NumFormat=%d, PrefetchCount=%d\n",
...
//		XE_PRINT("specified swizzle: %08x\n", fmt->e[j].swizzle);
...
//			XE_PRINT("comp%d sw=%c ", comp, "xyzw01?_"[sw]);
...
//		XE_PRINT("  new Offset=%08x, DataFormat=%d, Stride=%d, Signed=%d, NumFormat=%d, PrefetchCount=%d\n",
//			Offset,DataFormat, Stride, Signed, NumFormat, PrefetchCount);
//		XE_PRINT("  new vfetch: %08x %08x %08x\n", vfetch[0], vfetch[1], vfetch[2]);
	}
...
void Xe_Init(struct XenosDevice *xe)
{
	...
	kmeminit(&xe->mempool,vram,MEMPOOL_SIZE);
	
	gpu_regs = xe->regs = ioremap(0xec800000ULL, 0x20000, 1);
	if(!xe->regs) Xe_Fatal(xe,"Failed to map registers");
...
//	XE_PRINT("waiting for frameidx %08x\n", xe->frameidx);
	int timeout = 1<<24;
	do {
...
{
//	XE_PRINT("SetVertexShaderConstantF\n");
	memcpy(xe->alu_constants + start * 4, data, count * 16);
	Xe_DirtyAluConstant(xe, start, count);
//	while (count--)
//	{
//		XE_PRINT("%.3f %.3f %.3f %.3f\n", data[0], data[1], data[2], data[3]);
//		data += 4;
//	}
}
...
{
	start += 256;
//	XE_PRINT("SetPixelShaderConstantF (%d+)\n", start);
	memcpy(xe->alu_constants + start * 4, data, count * 16);
	Xe_DirtyAluConstant(xe, start, count);
//	while (count--)
//	{
